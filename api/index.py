...
# Point Flask at the project-root templates & static folders
...
def api_recent():
    ...
    if not token:
        return jsonify({"error": "not_authenticated"}), 401
    return jsonify(spotify_get("/me/player/recently-played", token, params={"limit": 50}))
...
def api_listening_streak():
    ...
    return jsonify({"streak": streak, "active_days": len(days),
...
def api_mood_scatter():
    ...
    for i, f in enumerate(feats):
        ...
        t = tracks[i]
        result.append({
            "name": t["name"], "artist": t["artists"][0]["name"] if t["artists"] else "",
            "valence": round(f.get("valence", 0), 3), "energy": round(f.get("energy", 0), 3),
...
def api_personality():
    ...
    energy = avg("energy"); valence = avg("valence"); dance = avg("danceability")
    acoustic = avg("acousticness"); instru = avg("instrumentalness"); speech = avg("speechiness")
    if speech > 0.35:
    ...
    p["scores"] = {"Energy": round(energy*100), "Valence": round(valence*100),
                   "Danceability": round(dance*100), "Acousticness": round(acoustic*100)}
    return jsonify(p)
...
def api_recent_timeline():
    ...
    for item in data.get("items", []):
        ...
        items.append({
            ...
            "image": t["album"]["images"][0]["url"] if t["album"]["images"] else None,
            "played_at": item["played_at"], "date": dt.strftime("%b %d"),
            "time": dt.strftime("%H:%M"), "weekday": dt.strftime("%a"),
        })
...
def api_decade_breakdown():
    ...
    for t in data.get("items", []):
        ...
        if rd and len(rd) >= 4:
            decade = (int(rd[:4]) // 10) * 10
            decades[decade] += 1
    return jsonify([{"decade": f"{d}s", "count": c} for d, c in sorted(decades.items())])
